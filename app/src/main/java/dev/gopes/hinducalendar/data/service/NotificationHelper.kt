...
import dev.gopes.hinducalendar.domain.model.NotificationTime
...
class NotificationHelper @Inject constructor(
    ...
    /**
     * Schedules the daily festival reminder worker.
     * Runs once per day at 6 AM to check for upcoming festivals
     * and post reminders based on the user's timing preferences.
     */
    fun scheduleFestivalWorker() {
        val now = Calendar.getInstance()
        val target = Calendar.getInstance().apply {
        ...
        val festivalWorkRequest = PeriodicWorkRequestBuilder<FestivalReminderWorker>(
            1, TimeUnit.DAYS
        )
            .setInitialDelay(initialDelayMillis, TimeUnit.MILLISECONDS)
            .addTag(FestivalReminderWorker.WORK_NAME)
            .build()

        WorkManager.getInstance(context).enqueueUniquePeriodicWork(
            FestivalReminderWorker.WORK_NAME,
            ExistingPeriodicWorkPolicy.UPDATE,
            festivalWorkRequest
        )
    }
    ...
    /**
     * Cancels the festival reminder worker.
     */
    fun cancelFestivalWorker() {
        WorkManager.getInstance(context).cancelUniqueWork(FestivalReminderWorker.WORK_NAME)
    }
}
